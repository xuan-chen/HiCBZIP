# score_fast_higashi_10runs_varyseed.R
# Run with: Rscript score_fast_higashi_10runs_varyseed.R
...
if (!nzchar(score_bin)) {
  fallback_candidates <- c(
    "/home/xchen/miniconda3/envs/score_higashi_py39_clean/bin/score",
...
out_root <- file.path("results", "oocyte_zygote_fast_higashi_10runs")
...
run_score_cmd <- function(score_bin, args, label, stdout_file, stderr_file) {
  cat("\n[", label, "]\n", sep = "")
...
for (i in seq_len(n_runs)) {
  ...
  if (force_rerun || !file.exists(metrics_json)) {
    ...
    run_score_cmd(
      score_bin,
      args,
      label = paste("score embed fast_higashi run", i),
