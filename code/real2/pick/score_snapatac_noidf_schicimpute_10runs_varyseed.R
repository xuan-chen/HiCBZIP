# score_snapatac_noidf_schicimpute_10runs_varyseed.R
# Run with: Rscript score_snapatac_noidf_schicimpute_10runs_varyseed.R
...
scool_path <- file.path("data", "schicimpute_score_compare_from_pairs", "oocyte_zygote_schicimpute_1M.scool")
ref_path <- file.path("data", "schicimpute_score_compare_from_pairs", "oocyte_zygote_ref_min_depth_5000.tsv")
if (!file.exists(scool_path)) stop("Missing scHiCImpute scool: ", scool_path)
if (!file.exists(ref_path)) stop("Missing scHiCImpute reference: ", ref_path)
...
out_root <- file.path("results", "oocyte_zygote_schicimpute_10runs_snapatac_noidf")
...
for (i in seq_len(n_runs)) {
  ...
  dset_name <- sprintf("oocyte_zygote_schicimpute_pairs_r%02d", i)
  ...
  if (force_rerun || !file.exists(metrics_json)) {
    ...
    run_score_cmd(score_bin, args, label = paste("score embed scHiCImpute SnapATAC noIDF run", i))
  } else {
    cat("\n[skip] Reusing existing:", metrics_json, "\n")
  }

  out <- tibble(input_code = "schicimpute", input_label = "scHiCImpute", run = i, seed = seed_i)
