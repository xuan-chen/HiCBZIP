...
get_EBE_Matched_N = function(Y, c_plus_d = 10, lambda = 1, fix_negative_w = T){
  ...
  options(mc.cores = parallel::detectCores())  # to speed up sampling
  
  fit_normal <- stan(
    file = "C:/Users/67402/OneDrive - University of Miami/courses/research/HiC/STAN_BHZIP_lambda/BHZIP_match_normal_250906.stan",
    data = data_list_normal,
    chains     = 4,
    iter       = 2000,
    warmup     = 1000,
    seed       = 12345,
  ...
  muS_posterior = summary(fit_normal, c("mu_tilde"))$summary[,1]
  mu_posterior = summary(fit_normal, c("mu"))$summary[,1]
  pi_posterior = summary(fit_normal, c("pi"))$summary[,1]
  S_posterior = summary(fit_normal, c("S"))$summary[,1]
  ebe_normal = 
    list(
...
get_EBE_Matched_N_cmd = function(Y, c_plus_d = 10, lambda = 1, fix_negative_w = T, stan_mod){
  ...
  stan_fit_full = mod$sample(data = data_list_normal,seed = 12345,
